...
var LazyScroll = function (_LazyLoad) {
	...
	function LazyScroll(selector) {
		...
		if (!('IntersectionObserver' in window)) {
			...
			_this.observer = new window.IntersectionObserver(onIntersection.bind(_this), {
				rootMargin: '0px 0px',
				threshold: 0
			});

			_this.images.forEach(function (imageObj) {
				return _this.observer.observe(imageObj.image);
			});
		}
		return _this;
	}
...
function onIntersection(entries) {
	...
	var imagesToLoad = entries.reduce(function (images, entry) {
		if (entry.intersectionRatio > 0) {
			...
			var image = unloadedImages.find(function (i) {
				return i.image === target;
			});
...
function loadImages(imagesToLoad) {
	...
	imagesToLoad.forEach(function (lazyImage) {
		_this3.fireLazyLoadEvent(lazyImage.image);
...
function getWindowBoundaries() {
	...
	var xMax = pageXOffset + window.innerWidth;
	var yMin = pageYOffset;
	var yMax = pageYOffset + window.innerHeight;
